 ...
 * to be thrown.</p>
 */
@SuppressWarnings({"checkstyle:FinalLocalVariable", "CheckStyle", "unused"})
public final class MemoryCopy {
  ...
  public static void copyFromArray(
    ...
    if (order == nativeByteOrder) {
      ...
      copySwap(srcSegmentSlice, dstSegmentSlice, arrLayout16N, segLayout16NN);
    }
  }
  ...
  public static void copyToArray(
    ...
    if (order == nativeByteOrder) {
      ...
      copySwap(srcSegmentSlice, dstSegmentSlice, segLayout16NN, arrLayout16N);
    }
  }
  ...
  public static void copyFromArray(
    ...
    if (order == nativeByteOrder) {
      ...
      copySwap(srcSegmentSlice, dstSegmentSlice, arrLayout64N, segLayout64NN);
    }
  }
  ...
  public static void copyToArray(
    ...
    if (order == nativeByteOrder) {
      ...
      copySwap(srcSegmentSlice, dstSegmentSlice, segLayout64NN, arrLayout64N);
    }
  }
  ...
  public static void copyFromArray(
    ...
    if (order == nativeByteOrder) {
      ...
      copySwap(srcSegmentSlice, dstSegmentSlice, arrLayout32N, segLayout32NN);
    }
  }
  ...
  public static void copyToArray(
    ...
    if (order == nativeByteOrder) {
      ...
      copySwap(srcSegmentSlice, dstSegmentSlice, segLayout32NN, arrLayout32N);
    }
  }
  ...
  /**
   * This only exists until the new copyFrom exists in MemorySegment.
   * @param src the source segment
   * @param dst the destination segment
   * @param srcLayout the source layout
   * @param dstLayout the destination layout
   */
  private static void copySwap(MemorySegment src, MemorySegment dst, ValueLayout srcLayout, ValueLayout dstLayout) {
    int unitBytes = (int) (srcLayout.bitSize() / 8L); //bytes per multibyte primitive
    boolean swap = srcLayout.order() != dstLayout.order();

    if (unitBytes == 1 || !swap) {
      dst.copyFrom(src);
      return;
    }
    byte[] tmpArr = dst.toByteArray();
    MemorySegment tmpDst = MemorySegment.ofArray(tmpArr);
    int lenUnits = (int) src.byteSize() / unitBytes;

    if (unitBytes == 2) {
      for (int i = 0; i < lenUnits; i++) {
        MemoryAccess.setCharAtIndex(tmpDst, i, nonNativeByteOrder, MemoryAccess.getCharAtIndex(src, i));
      }
      dst.copyFrom(tmpDst);

    } else if (unitBytes == 4) {
      for (int i = 0; i < lenUnits; i++) {
        MemoryAccess.setIntAtIndex(tmpDst, i, nonNativeByteOrder, MemoryAccess.getIntAtIndex(src, i));
      }
      dst.copyFrom(tmpDst);
    } else {
      for (int i = 0; i < lenUnits; i++) {
        MemoryAccess.setLongAtIndex(tmpDst, i, nonNativeByteOrder, MemoryAccess.getLongAtIndex(src, i));
      }
      dst.copyFrom(tmpDst);
    }
  }
